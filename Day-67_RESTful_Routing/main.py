...
@app.route('/')
def get_all_posts():
    response = db.session.execute(db.select(BlogPost))
...
def create_post():
    ...
    if form.validate_on_submit():
        ...
        new_post = BlogPost(
            title=form.blog_title.data, subtitle=form.subtitle.data,
            date=post_date, body=form.body.data, author=form.author_name.data,
...
def edit_post(post_id):
    ...
    edit_form = PostForm(
        blog_title=post.title, subtitle=post.subtitle, img_url=post.img_url,
        author_name=post.author, body=post.body
    )
